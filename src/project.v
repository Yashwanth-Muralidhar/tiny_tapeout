...
module tt_um_vinayaka_pqc_fo (
  ...
  always @(*) begin
    case (pr)
      2'd1:    begin du=4'd10; dv=4'd4; c1_len=12'd960;  cx_len=12'd1088; n_tot=12'd1024; end
      2'd2:    begin du=4'd11; dv=4'd5; c1_len=12'd1408; cx_len=12'd1568; n_tot=12'd1280; end
      default: begin du=4'd10; dv=4'd4; c1_len=12'd640;  cx_len=12'd768;  n_tot=12'd768;  end
    endcase
  end
  ...
  // ---- hoisted: compress result decode -----------------------------------
  reg [11:0] dmask;
  always @(*) case (dop)
    4'd1:    dmask = 12'h001;
    4'd4:    dmask = 12'h00F;
  ...
  reg [11:0] pres, ynew;
  always @(*) case (dunp)
    4'd4:    begin accsh = acc[15:4];  rndb = acc[3];
             pres = {buf_r[3:0],  8'd0}; ynew = {8'd0, buf_r[3:0]};  end
    4'd5:    begin accsh = acc[16:5];  rndb = acc[4];
             pres = {buf_r[4:0],  7'd0}; ynew = {7'd0, buf_r[4:0]};  end
    4'd10:   begin accsh = acc[21:10]; rndb = acc[9];
             pres = {buf_r[9:0],  2'd0}; ynew = {2'd0, buf_r[9:0]};  end
    default: begin accsh = acc[22:11]; rndb = acc[10];
             pres = {buf_r[10:0], 1'd0}; ynew = {1'd0, buf_r[10:0]}; end
  endcase
  ...
  always @(posedge clk) begin
    if (!rst_n) begin
      st <= S_IDLE; wr_q <= 0; start_q <= 0; rd_q <= 0;
      acc <= 0; rem <= 0; scan <= 0; buf_r <= 0; nbits <= 0;
      byte_cnt <= 0; coef_cnt <= 0; ycoef <= 0; vco <= 0; aux <= 0;
      bitk <= 0; cfull <= 0; diff <= 0; out_reg <= 0; out_cnt <= 0;
      ...
      if (start_p) begin
        st <= S_RXC; buf_r <= 0; nbits <= 0; byte_cnt <= 0; coef_cnt <= 0;
        diff <= 0; fault <= 0; match_r <= 0; in_c2 <= 0;
        ...
        case (st)
          ...
          S_DEC: begin
            ...
            scan <= {scan[10:0], 1'b0};
            if (bitk == dunp - 4'd1) st <= S_OUT;
            else bitk <= bitk + 4'd1;
          end

          S_OUT: begin
            if (in_c2) begin
              vco <= dres; aux_hi <= 0; st <= S_RXA;
            end else begin
              out_reg  <= {4'd0, dres};
              out_cnt  <= 2'd2;
          ...
          S_RXA: if (wr_p) begin
            if (!aux_hi) begin aux[7:0] <= ui_in; aux_hi <= 1; end
            else begin
              aux[11:8] <= ui_in[3:0]; aux_hi <= 0;
          ...
          S_CLD: begin
            rem <= cx; bitk <= dop; cfull <= 0; st <= S_CMP;
          end

          S_CMP: begin
            rem   <= ge ? (rem2[11:0] - Q) : rem2[11:0];
            cfull <= {cfull[11:0], ge};
            if (bitk == 0) st <= S_ACC; else bitk <= bitk - 4'd1;
          end

          S_ACC: begin
            if (phase) begin
              diff     <= diff | (ycoef ^ cval);
              coef_cnt <= coef_cnt + 12'd1;
              st       <= S_UNP;
