...
def aux_req(dut):
    # uio_out[5] = 1 exactly when DUT is in S_RXA.
...
def stname(dut):
    try:
        state_names = [
            "IDLE", "RXC", "UNP", "DEC", "OUT", "RXA",
            "MSUB", "CLD", "CMP", "ACC", "ACC2", "FIN", "DONE"
        ]
...
async def start_clock(dut):
    cocotb.start_soon(
        Clock(dut.clk, CLOCK_NS, unit="ns").start()
    )
...
async def service_pass2(dut, regenerated, coeff_idx, limit=10000):
    """
    Service the DUT until it is waiting for the next ciphertext byte
    or has reached DONE.

    AUX_REQ is a real top-level signal:
        uio_out[5] = 1 -> S_RXA
    """
    ...
    for _ in range(limit):
        ...
        if aux_req(dut):
            assert coeff_idx < len(regenerated), (
                f"DUT requested too many auxiliary coefficients: "
                f"{coeff_idx} >= {len(regenerated)}"
            )

            await feed_aux(
                dut,
                regenerated[coeff_idx]
            )
    ...
    raise AssertionError(
        f"Timeout servicing pass-2 DUT: "
        f"coeff_idx={coeff_idx}/{len(regenerated)}, "
        f"busy={busy(dut)}, "
        f"aux_req={aux_req(dut)}, "
...
async def run_pass2(dut, param, seed, tamper_index=None):
    ...
    ciphertext, regenerated = make_case(
        param,
        seed,
        tamper_index
    )
    ...
    for byte in ciphertext:
        ...
        # First service anything already pending.
        coeff_idx = await service_pass2(
            dut,
            regenerated,
            coeff_idx
        )

        # Provide the next ciphertext byte.
        await pulse_wr(
            dut,
            byte
        )

        # The byte may have caused one or more AUX requests.
        coeff_idx = await service_pass2(
            dut,
            regenerated,
            coeff_idx
    ...
    # Service any final coefficient request.
    coeff_idx = await service_pass2(
        dut,
        regenerated,
        coeff_idx
    ...
    assert coeff_idx == p["n_tot"], (
        f"{p['name']}: sent {coeff_idx} auxiliary coefficients, "
        f"expected {p['n_tot']}"
    )
...
async def run_pass1(dut, param, seed):
    ...
    async def drain():
        while True:
            ...
            is_gl = (
                os.getenv("GATES", "").lower()
                == "yes"
            )

            if not is_gl and hasattr(
                dut.user_project,
                "st"
            ):
                st = int(
                    dut.user_project.st.value
                )

                if st == 5:
                    raise AssertionError(
                        f"{p['name']} pass1: "
                        f"unexpected S_RXA request"
                    )
                ...
                # Gate-level mode:
                # bit 5 is AUX_REQ and should remain 0
                # during pass-1.
                if not busy(dut):
